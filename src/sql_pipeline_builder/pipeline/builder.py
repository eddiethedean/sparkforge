...
class SqlPipelineBuilder(BasePipelineBuilder):
    """
    ...
    def __init__(
        ...
        *,
        ...
        # but we need to type them correctly for SQL steps
    ...
    def add_silver_transform(
        ...
        *,
        ...
        # Use base class method for dependency validation
        try:
            # Create a temporary step object for validation
            temp_step = type(
                "TempStep", (), {"name": name, "source_bronze": source_bronze}
            )()
            self._validate_step_dependencies(temp_step, "silver", self.bronze_steps)
        except Exception as e:
            raise ValidationError(str(e)) from e
        ...
        *,
        ...
        if source_silvers:
            try:
                # Create a temporary step object for validation
                temp_step = type(
                    "TempStep", (), {"name": name, "source_silvers": source_silvers}
                )()
                self._validate_step_dependencies(temp_step, "gold", self.silver_steps)
            except Exception as e:
                raise ValidationError(str(e)) from e
        if model_class is None or not hasattr(model_class, "__table__"):
            raise ValidationError(
    ...
    def to_pipeline(self) -> SqlPipelineRunner:
        ...
        if not self.bronze_steps and not self.silver_steps and not self.gold_steps:
            raise ValidationError(
                "Pipeline must have at least one step (Bronze, Silver, or Gold)"
            )
