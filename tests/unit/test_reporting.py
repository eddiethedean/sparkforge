...
class TestReportingIntegration:
    ...
    def test_all_functions_return_dicts(self):
        ...
        transform_dict = create_transform_dict(
            input_rows=1000, output_rows=950, duration_secs=300.0,
            skipped=False, start_at=start_at, end_at=end_at
        )
        assert isinstance(transform_dict, dict)
        
        # Test create_write_dict
        write_dict = create_write_dict(
            mode="append", rows=1000, duration_secs=300.0,
            table_fqn="test.table", skipped=False,
        ...
        summary_dict = create_summary_report(
            total_steps=5, successful_steps=4, failed_steps=1,
            total_duration=300.0, total_rows_processed=1000,
            total_rows_written=950, avg_validation_rate=95.0
        )
