...
class TestWriteOverwriteTable:
    ...
    def test_write_overwrite_table_with_options(self):
        ...
        with patch('sparkforge.table_operations.logger'):
            result = write_overwrite_table(
                mock_df, "test_schema.test_table",
                compression="snappy", partitionBy="date"
            )
    ...
    def test_write_overwrite_table_failure(self):
        ...
        with pytest.raises(TableOperationError, match="Failed to write table test_schema.test_table: Cache failed"):
    ...
    def test_write_append_table_with_options(self):
        ...
        with patch('sparkforge.table_operations.logger'):
            result = write_append_table(
                mock_df, "test_schema.test_table",
                compression="gzip", maxRecordsPerFile=1000
            )
...
class TestReadTable:
    ...
    def test_read_table_analysis_exception(self):
        ...
        with pytest.raises(TableOperationError, match="Failed to read table test_schema.test_table: Table not found"):
    ...
    def test_read_table_other_exception(self):
        ...
        with pytest.raises(TableOperationError, match="Failed to read table test_schema.test_table: Connection failed"):
            read_table(mock_spark, "test_schema.test_table")
...
class TestTableExists:
    ...
    def test_table_exists_false_analysis_exception(self):
        ...
        with patch('sparkforge.table_operations.logger') as mock_logger:
            ...
            mock_logger.debug.assert_called_once_with("Table test_schema.test_table does not exist (AnalysisException)")
    ...
    def test_table_exists_false_other_exception(self):
        ...
        with patch('sparkforge.table_operations.logger') as mock_logger:
            ...
            assert result is False
            mock_logger.warning.assert_called_once_with("Error checking if table test_schema.test_table exists: Connection failed")
...
class TestDropTable:
    ...
    def test_drop_table_success(self):
        ...
        mock_jspark_session.sharedState.return_value.externalCatalog.return_value = mock_external_catalog
        ...
        with patch('sparkforge.table_operations.table_exists', return_value=True):
            with patch('sparkforge.table_operations.logger'):
                ...
                assert result is True
                mock_external_catalog.dropTable.assert_called_once_with("test_schema", "test_table", True, True)
    ...
    def test_drop_table_with_default_schema(self):
        ...
        with patch('sparkforge.table_operations.table_exists', return_value=True):
            with patch('sparkforge.table_operations.logger'):
                ...
                assert result is True
                mock_external_catalog.dropTable.assert_called_once_with("default", "test_table", True, True)
    ...
    def test_drop_table_failure(self):
        ...
        with patch('sparkforge.table_operations.table_exists', return_value=True):
            with patch('sparkforge.table_operations.logger') as mock_logger:
                ...
                assert result is False
                mock_logger.warning.assert_called_once_with("Failed to drop table test_schema.test_table: Drop failed")
    
    def test_drop_table_exception_during_check(self):
        """Test table drop when table_exists check fails."""
        mock_spark = MagicMock()
        
        with patch('sparkforge.table_operations.table_exists', side_effect=Exception("Check failed")):
            with patch('sparkforge.table_operations.logger') as mock_logger:
                result = drop_table(mock_spark, "test_schema.test_table")
                
                assert result is False
                mock_logger.warning.assert_called_once_with("Failed to drop table test_schema.test_table: Check failed")
