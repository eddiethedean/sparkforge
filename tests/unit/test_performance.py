...
class TestFormatDuration:
    ...
    def test_format_duration_seconds(self):
        ...
        assert format_duration(59.9) == "59.90s"  # Changed from 59.999 to avoid rounding to 60.00s
    ...
    def test_format_duration_minutes(self):
        ...
        assert format_duration(3599.0) == "59.98m"  # Use exact value to avoid rounding issues
    ...
    def test_format_duration_negative(self):
        ...
        assert format_duration(-60) == "-60.00s"  # Negative values are treated as seconds
        assert format_duration(-3600) == "-3600.00s"  # Negative values are treated as seconds
...
class TestPerformanceMonitor:
    ...
    def test_performance_monitor_with_max_duration_warning(self):
        ...
        with patch('sparkforge.performance.logger') as mock_logger:
            with performance_monitor("test operation", max_duration=0.001):  # Very short threshold
                time.sleep(0.01)  # Longer than threshold
        
            assert mock_logger.info.call_count == 2  # Start and completion
            assert mock_logger.warning.call_count == 1  # Warning about exceeding threshold
    ...
    def test_performance_monitor_with_max_duration_no_warning(self):
        ...
        with patch('sparkforge.performance.logger') as mock_logger:
            with performance_monitor("test operation", max_duration=1.0):  # Long threshold
...
class TestTimeWriteOperation:
    ...
    def test_time_write_operation_invalid_mode(self):
        ...
        with patch('sparkforge.performance.logger'):
            with pytest.raises(ValueError, match="Unknown write mode 'invalid'. Supported modes: overwrite, append"):
