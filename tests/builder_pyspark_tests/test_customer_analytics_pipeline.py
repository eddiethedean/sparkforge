...
class TestCustomerAnalyticsPipeline:
    ...
    def test_complete_customer_360_pipeline_execution(
        ...
        def customer_behavior_transform(spark, df, silvers):
            ...
            return (
                df.withColumn("timestamp_parsed", df.timestamp.cast("timestamp"))
                .withColumn("hour_of_day", df.timestamp_parsed.cast("timestamp").hour)
                .withColumn(
                    "day_of_week", df.timestamp_parsed.cast("timestamp").dayofweek
                )
                ...
                return (
                    ...
                    .withColumn(
                        ...
                        df.when(
                            ...
                            & (df.value_segment.isin(["premium", "high_value"])),
                            "champions",
                        )
                        .when(
                            (df.engagement_level == "high")
                            & (df.value_segment == "medium_value"),
                            "loyal_customers",
                        )
                        .when(
            ...
            if segments is not None:
                return (
                    ...
                    .agg(
                        ...
                        F.col("lifetime_value").avg().alias("avg_lifetime_value"),
                        F.col("lifetime_value").sum().alias("total_value"),
        ...
        def churn_features_transform(spark, df, silvers):
            ...
            return (
                ...
                .withColumn(
                    ...
                    df.current_date().datediff(df.timestamp_parsed),
                )
                .withColumn(
                    "is_support_interaction", df.interaction_type == "support_ticket"
                )
        ...
        def churn_prediction_transform(spark, silvers):
            ...
            if features is not None and customers is not None:
                ...
                return (
                    ...
                    .withColumn(
                        "churn_probability",
                        F.when(F.col("max_risk_factors") >= 2, 0.8)
                        .when(F.col("avg_days_inactive") > 14, 0.6)
                        .when(F.col("support_tickets") > 0, 0.4)
                        .otherwise(0.1),
                    )
                    .withColumn(
                        "churn_risk_level",
    ...
    def test_customer_lifetime_value_analysis(
        ...
        def customer_ltv_transform(spark, df, silvers):
            ...
            return (
                df.withColumn("order_date_parsed", df.order_date.cast("date"))
                ...
                .withColumn(
                    ...
                    df.current_date().datediff(df.first_order_date),
                )
                .withColumn(
                    "order_frequency",
                    df.total_orders / F.greatest(df.customer_tenure_days, 1),
                )
            )
