...
class TestEcommercePipeline:
    ...
    def test_incremental_order_processing(
        ...
        builder.add_gold_transform(
            name="order_summary",
            transform=lambda spark, silvers: silvers["clean_orders"].select(
                F.lit("summary").alias("type"), F.count("*").alias("total_orders")
            ),
    ...
    def test_logging_and_monitoring(
        ...
        builder.add_gold_transform(
            name="order_metrics",
            transform=lambda spark, silvers: silvers["clean_orders"].select(
                F.lit("metrics").alias("type"), F.count("*").alias("total_orders")
            ),
            rules={"type": ["not_null"], "total_orders": ["not_null"]},
