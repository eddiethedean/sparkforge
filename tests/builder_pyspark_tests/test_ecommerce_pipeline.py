...
and sales analytics using PySpark.
"""
...
class TestEcommercePipeline:
    ...
    def test_complete_ecommerce_pipeline_execution(self, spark_session, data_generator, test_assertions):
        ...
        customers_df = data_generator.create_customer_data(spark_session, num_customers=25)
        ...
        def clean_orders_transform(spark, df, silvers):
            ...
            return (df
                .filter(F.col("status").isin(["shipped", "delivered"]))  # Only successful orders
                ...
                .select("order_id", "customer_id", "product_id", "quantity", 
                       "unit_price", "total_amount", "order_date_parsed", "status")
            )
        ...
        def enrich_customers_transform(spark, df, silvers):
            ...
            return (df
                .withColumn("registration_date_parsed", F.to_date("registration_date"))
                .withColumn("is_premium", F.col("segment") == "premium")
                .select("customer_id", "name", "email", "country", "segment",
                       "registration_date_parsed", "is_premium", "total_orders", "lifetime_value")
            )
        ...
        def daily_sales_analytics_transform(spark, silvers):
            ...
            if clean_orders is not None:
                ...
                return spark.createDataFrame([], ["order_date_parsed", "total_orders", "daily_revenue", "avg_order_value", "unique_customers"])
        ...
        def customer_lifetime_value_transform(spark, silvers):
            ...
            if clean_orders is not None and enriched_customers is not None:
                ...
                return (enriched_customers
                    ...
                    .select(
                        "customer_id", "name", "country", "segment", "is_premium",
                        F.coalesce(F.col("order_count"), F.lit(0)).alias("actual_orders"),
                        F.coalesce(F.col("total_spent"), F.lit(0.0)).alias("actual_spent"),
                        F.coalesce(F.col("avg_order_value"), F.lit(0.0)).alias("actual_avg_order"),
                        "lifetime_value"
                    )
                )
            else:
                return spark.createDataFrame([], ["customer_id", "name", "country", "segment", "is_premium", "actual_orders", "actual_spent", "actual_avg_order", "lifetime_value"])
    ...
    def test_incremental_order_processing(self, spark_session, data_generator, test_assertions):
        """Test incremental processing of new orders."""
        
        # Create initial data
        initial_orders = data_generator.create_ecommerce_orders(spark_session, num_orders=20)
        ...
        builder.add_silver_transform(
            ...
            transform=lambda spark, df, silvers: df.filter(F.col("status") == "delivered"),
            rules={"order_id": ["not_null"]},
            table_name="clean_orders"
        )
        ...
        builder.add_gold_transform(
            name="order_summary",
            transform=lambda spark, silvers: silvers["clean_orders"].select(F.lit("summary").alias("type"), F.count("*").alias("total_orders")),
            rules={"type": ["not_null"], "total_orders": ["not_null"]},
        ...
        new_orders = data_generator.create_ecommerce_orders(spark_session, num_orders=10)
    ...
    def test_validation_failures(self, spark_session, test_assertions):
        ...
        bad_orders = spark_session.createDataFrame([
            ("ORD-001", "CUST-001", None, 1, 10.0, "2024-01-01", "delivered"),  # Missing product_id
            ("ORD-002", "CUST-002", "PROD-001", -1, 10.0, "2024-01-01", "delivered"),  # Negative quantity
            ("ORD-003", "CUST-003", "PROD-001", 1, -5.0, "2024-01-01", "delivered"),  # Negative price
    ...
    def test_logging_and_monitoring(self, spark_session, data_generator, log_writer_config, test_assertions):
        ...
        builder.add_gold_transform(
            name="order_metrics",
            transform=lambda spark, silvers: silvers["clean_orders"].select(F.lit("metrics").alias("type"), F.count("*").alias("total_orders")),
            rules={"type": ["not_null"], "total_orders": ["not_null"]},
