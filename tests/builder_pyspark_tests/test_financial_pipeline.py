...
class TestFinancialPipeline:
    ...
    def test_complete_financial_transaction_pipeline_execution(self, spark_session, data_generator, test_assertions):
        """Test complete financial pipeline: transactions → validation → fraud detection → compliance."""
        
        # Create realistic financial data
        transactions_df = data_generator.create_financial_transactions(spark_session, num_transactions=100)
        ...
        def validate_transactions_transform(spark, df, silvers):
            ...
            return (df
                .withColumn("timestamp_parsed", F.col("timestamp").cast("timestamp"))
        ...
        def enrich_accounts_transform(spark, df, silvers):
            ...
            return (df
                .withColumn("opening_date_parsed", F.col("opening_date").cast("date"))
                .withColumn("account_age_days", F.current_date().cast("date").cast("long") - F.col("opening_date_parsed").cast("long"))
    ...
    def test_fraud_detection_scenarios(self, spark_session, data_generator, test_assertions):
    ...
    def test_compliance_monitoring(self, spark_session, data_generator, test_assertions):
        ...
        def compliance_monitoring_transform(spark, silvers):
            ...
            if transactions is not None:
                return (transactions
                    .withColumn("is_large_transaction", ["gte", 10000])
                    .withColumn("is_suspicious_pattern", 
                              (["gte", 10000]) & (df.category == "transfer"))
                    .withColumn("compliance_flag",
                              df.when(["gte", 15000], "SAR_REQUIRED")  # Suspicious Activity Report
                              .when(["gte", 10000], "MONITOR")
                              .otherwise("NORMAL"))
    ...
    def test_financial_audit_logging(self, spark_session, data_generator, log_writer_config, test_assertions):
        """Test comprehensive audit logging for financial pipeline."""
        
        # Create test data
        transactions_df = data_generator.create_financial_transactions(spark_session, num_transactions=50)
