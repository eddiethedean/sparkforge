...
class TestDeltaLakeComprehensive:
    ...
    def test_delta_lake_acid_transactions(self, delta_delta_spark_session):
        """Test ACID transaction properties of Delta Lake."""
        # Create initial data
        data = [(1, "Alice", "2024-01-01"), (2, "Bob", "2024-01-02")]
        df = delta_delta_spark_session.createDataFrame(data, ["id", "name", "date"])
        ...
        initial_count = delta_spark_session.table(table_name).count()
        assert initial_count == 2
        
        # Test transaction - add more data
        new_data = [(3, "Charlie", "2024-01-03"), (4, "Diana", "2024-01-04")]
        new_df = delta_spark_session.createDataFrame(new_data, ["id", "name", "date"])
        new_df.write.format("delta").mode("append").saveAsTable(table_name)
        
        # Verify transaction completed
        final_count = delta_spark_session.table(table_name).count()
        assert final_count == 4
        
        # Clean up
        delta_spark_session.sql(f"DROP TABLE IF EXISTS {table_name}")
    
    def test_delta_lake_schema_evolution(self, delta_spark_session):
        """Test schema evolution capabilities."""
        # Create initial schema
        initial_data = [(1, "Alice"), (2, "Bob")]
        initial_df = delta_spark_session.createDataFrame(initial_data, ["id", "name"])
        ...
        evolved_df = delta_spark_session.createDataFrame(evolved_data, ["id", "name", "age"])
        ...
        result_df = delta_spark_session.table(table_name)
    ...
    def test_delta_lake_time_travel(self, delta_spark_session):
        ...
        version_0 = delta_spark_session.read.format("delta").option("versionAsOf", 0).table(table_name)
        ...
        version_0_again = delta_spark_session.read.format("delta").option("versionAsOf", 0).table(table_name)
        assert version_0_again.count() == 2
        
        # Verify current version has more data
        current_version = delta_spark_session.table(table_name)
        assert current_version.count() == 3
        
        # Clean up
        delta_spark_session.sql(f"DROP TABLE IF EXISTS {table_name}")
    
    def test_delta_lake_merge_operations(self, delta_spark_session):
        """Test MERGE operations (upsert functionality)."""
        # Create target table
        target_data = [(1, "Alice", 100), (2, "Bob", 200)]
        target_df = delta_spark_session.createDataFrame(target_data, ["id", "name", "score"])
        ...
        source_df = delta_spark_session.createDataFrame(source_data, ["id", "name", "score"])
        ...
        delta_spark_session.sql(merge_sql)
        
        # Verify merge results
        result_df = delta_spark_session.table("test_schema.delta_merge_target")
        ...
        # Clean up
        delta_spark_session.sql("DROP TABLE IF EXISTS test_schema.delta_merge_target")
        delta_spark_session.sql("DROP TABLE IF EXISTS test_schema.delta_merge_source")
    
    def test_delta_lake_optimization(self, delta_spark_session):
        ...
        delta_spark_session.sql(f"OPTIMIZE {table_name}")
        
        # Test VACUUM command (retention period in hours) - use 168 hours minimum
        delta_spark_session.sql(f"VACUUM {table_name} RETAIN 168 HOURS")
    ...
    def test_delta_lake_history_and_metadata(self, delta_spark_session):
        """Test Delta Lake history and metadata operations."""
        # Create table
        data = [(1, "Alice"), (2, "Bob")]
        df = delta_spark_session.createDataFrame(data, ["id", "name"])
        table_name = "test_schema.delta_history"
        
        df.write.format("delta").mode("overwrite").saveAsTable(table_name)
        
        # Test DESCRIBE HISTORY
        history_df = delta_spark_session.sql(f"DESCRIBE HISTORY {table_name}")
        assert history_df.count() >= 1
        
        # Test DESCRIBE DETAIL
        detail_df = delta_spark_session.sql(f"DESCRIBE DETAIL {table_name}")
        assert detail_df.count() == 1
        
        # Test SHOW TBLPROPERTIES
        properties_df = delta_spark_session.sql(f"SHOW TBLPROPERTIES {table_name}")
        assert properties_df.count() > 0
        
        # Clean up
        delta_spark_session.sql(f"DROP TABLE IF EXISTS {table_name}")
    
    def test_delta_lake_concurrent_writes(self, delta_spark_session):
        ...
        final_df = delta_spark_session.table(table_name)
        assert final_df.count() == 16  # 1 initial + 3 * 5 concurrent
        
        # Clean up
        delta_spark_session.sql(f"DROP TABLE IF EXISTS {table_name}")
    
    def test_delta_lake_performance_characteristics(self, delta_spark_session):
        ...
        df = delta_spark_session.createDataFrame(data, ["id", "name", "date", "score"])
        ...
        start_time = time.time()
        delta_df = delta_spark_session.table(delta_table)
        ...
        delta_spark_session.sql(f"OPTIMIZE {delta_table}")
        
        # Test optimized read performance
        start_time = time.time()
        optimized_df = delta_spark_session.table(delta_table)
        ...
        # Clean up
        delta_spark_session.sql(f"DROP TABLE IF EXISTS {delta_table}")
    
    def test_delta_lake_data_quality_constraints(self, delta_spark_session):
        ...
        df = delta_spark_session.createDataFrame(data, ["id", "name", "age"])
        df.write.format("delta").mode("overwrite").saveAsTable(table_name)
        
        # Add constraints (if supported in this version)
        try:
            delta_spark_session.sql(f"ALTER TABLE {table_name} ADD CONSTRAINT age_positive CHECK (age > 0)")
            delta_spark_session.sql(f"ALTER TABLE {table_name} ADD CONSTRAINT id_unique CHECK (id IS NOT NULL)")
            
            # Test constraint violation
            invalid_data = [(3, "Charlie", -5)]  # Negative age should violate constraint
            invalid_df = delta_spark_session.createDataFrame(invalid_data, ["id", "name", "age"])
