...
class TestCustomerAnalyticsPipeline:
    ...
    def test_complete_customer_360_pipeline_execution(
        ...
        def customer_segments_transform(spark, df, silvers):
            ...
            if behavior_data is not None:
                ...
                customer_metrics = behavior_data.groupBy("customer_id").agg(
                ...
                return (
                    df.join(customer_metrics, "customer_id", "left")
                    .withColumn(
                        "engagement_level",
                    ...
                    .withColumn(
                        "behavior_segment",
                        F.when(
                            (F.col("engagement_level") == "high")
                            & (F.col("value_segment").isin(["premium", "high_value"])),
                            "champions",
                        )
                        .when(
                            (F.col("engagement_level") == "high")
                            & (F.col("value_segment") == "medium_value"),
                            "loyal_customers",
                        )
                        .when(
                            (F.col("engagement_level") == "medium")
                            & (F.col("value_segment").isin(["premium", "high_value"])),
                            "potential_champions",
                        )
                        .when(
                            (F.col("engagement_level") == "low")
                            & (F.col("value_segment") == "premium"),
                            "at_risk",
                        )
            ...
            if segments is not None and behavior is not None:
                ...
                recent_behavior = (
                    behavior.filter(
                        ...
                        >= F.current_date() - F.expr("INTERVAL 30 DAYS")
                    )
                    .groupBy("customer_id")
                    .agg(
                        F.count("*").alias("recent_interactions"),
                        F.col("engagement_score").sum().alias("recent_engagement"),
                ...
                # Create 360 view
                return (
                    segments.join(recent_behavior, "customer_id", "left")
                    .withColumn(
                        "churn_risk_score",
                        F.when(F.col("behavior_segment") == "at_risk", 0.9)
                        .when(F.col("recent_interactions") == 0, 0.7)
                        .when(F.col("recent_engagement") < 10, 0.5)
                        .otherwise(0.1),
                    )
                    .withColumn(
                        "next_best_action",
                        F.when(F.col("behavior_segment") == "champions", "upsell")
                        .when(
                            F.col("behavior_segment") == "at_risk", "retention_campaign"
                        )
                        .when(
                            F.col("behavior_segment") == "new_customers", "onboarding"
                        )
        ...
        def customer_analytics_dashboard_transform(spark, silvers):
            ...
            if segments is not None:
                return (
                    segments.groupBy(
                        "behavior_segment", "engagement_level", "value_segment"
                    )
                    .agg(
    ...
    def test_customer_churn_prediction(
        ...
        builder.with_bronze_rules(
            name="interactions", rules={"customer_id": ["not_null"]}
        )
        ...
        def churn_features_transform(spark, df, silvers):
            ...
            return (
                ...
                .withColumn(
                    ...
                    F.current_date().datediff(F.col("timestamp_parsed")),
                )
                .withColumn(
                    "is_support_interaction",
                    F.col("interaction_type") == "support_ticket",
                )
                .withColumn("low_engagement", F.col("duration_seconds") < 60)
                .withColumn(
                    "churn_risk_factors",
                    F.when(F.col("days_since_last_order") > 30, 1).otherwise(0)
                    + F.when(F.col("is_support_interaction"), 1).otherwise(0)
                    + F.when(F.col("low_engagement"), 1).otherwise(0),
                )
            )
        ...
        builder.add_silver_transform(
            ...
            rules={"customer_id": ["not_null"], "churn_risk_factors": ["non_negative"]},
        ...
        def churn_prediction_transform(spark, silvers):
            ...
            if features is not None and customers is not None:
                ...
                customer_risk = features.groupBy("customer_id").agg(
                    F.col("churn_risk_factors").max().alias("max_risk_factors"),
                    F.col("days_since_last_activity").avg().alias("avg_days_inactive"),
                    F.count("*").alias("total_interactions"),
                    F.col("is_support_interaction").sum().alias("support_tickets"),
                )

                return (
                    customers.join(customer_risk, "customer_id", "left")
                    .withColumn(
                        "churn_probability",
        ...
        builder.add_gold_transform(
            ...
            rules={"customer_id": ["not_null"], "churn_probability": ["not_null"]},
    ...
    def test_customer_lifetime_value_analysis(
        ...
        builder.with_bronze_rules(name="orders", rules={"customer_id": ["not_null"]})
        ...
        def customer_ltv_transform(spark, df, silvers):
            ...
            return (
                ...
                .agg(
                    ...
                    F.col("total_amount").sum().alias("total_spent"),
                    F.col("total_amount").avg().alias("avg_order_value"),
                    F.col("order_date_parsed").min().alias("first_order_date"),
                    F.col("order_date_parsed").max().alias("last_order_date"),
                    F.col("product_id").nunique().alias("unique_products"),
                )
                .withColumn(
                    "customer_tenure_days",
                    F.current_date().datediff(F.col("first_order_date")),
                )
                .withColumn(
                    "order_frequency",
                    F.col("total_orders")
                    / F.greatest(F.col("customer_tenure_days"), 1),
                )
            )
        ...
        builder.add_silver_transform(
            ...
            rules={"customer_id": ["not_null"], "total_spent": ["non_negative"]},
        ...
        def ltv_segments_transform(spark, silvers):
            ...
            if ltv_data is not None and customers is not None:
                ...
                ltv_stats = ltv_data.select(
                    F.percentile_approx("total_spent", 0.5).alias("median_ltv"),
                    F.percentile_approx("total_spent", 0.8).alias("p80_ltv"),
                    F.percentile_approx("total_spent", 0.95).alias("p95_ltv"),
                ).collect()[0]
                ...
                return (
                    customers.join(ltv_data, "customer_id", "left")
                    .withColumn(
                        "ltv_segment",
                        F.when(F.col("total_spent") >= p95_ltv, "champions")
                        .when(F.col("total_spent") >= p80_ltv, "high_value")
                        .when(F.col("total_spent") >= median_ltv, "medium_value")
                        .otherwise("low_value"),
                    )
                    .withColumn(
                        "growth_potential",
                        F.when(
                            (F.col("order_frequency") < 0.1)
                            & (F.col("total_spent") > 0),
                            "high",
                        )
                        .when(
                            (F.col("order_frequency") < 0.2)
                            & (F.col("total_spent") > 0),
                            "medium",
                        )
                        .otherwise("low"),
        ...
        builder.add_gold_transform(
            ...
            rules={"customer_id": ["not_null"], "ltv_segment": ["not_null"]},
        ...
        # Execute pipeline
        result = pipeline.run_initial_load(
            bronze_sources={"customers": customers_df, "orders": orders_df}
        )
    ...
    def test_customer_analytics_logging(
        ...
        builder.add_gold_transform(
            ...
            transform=lambda spark, silvers: silvers["customer_behavior"].agg(
