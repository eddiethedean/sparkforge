...
class TestMultiSourcePipeline:
    ...
    def test_complete_multi_source_integration_pipeline_execution(self, mock_spark_session, data_generator, test_assertions):
        ...
        def clean_crm_data_transform(spark, df, silvers):
            ...
            return (df
                .withColumn("registration_date_parsed", df.registration_date.cast("date"))
                .withColumn("is_premium", df.tier == "premium")
                .withColumn("customer_age_days", df.current_date().datediff(df.registration_date_parsed))
                .withColumn("has_support_history", ["gt", 0])
    ...
    def test_complex_dependency_handling(self, mock_spark_session, data_generator, test_assertions):
        ...
        orders_df = data_generator.create_ecommerce_orders(mock_spark_session, num_orders=20)
    ...
    def test_multi_source_logging(self, mock_spark_session, data_generator, log_writer_config, test_assertions):
        """Test comprehensive logging for multi-source pipeline."""
        
        # Create test data from multiple sources
        crm_data = data_generator.create_customer_data(mock_spark_session, num_customers=5)
        erp_data = data_generator.create_ecommerce_orders(mock_spark_session, num_orders=10)
        ...
        assert mock_spark_session.storage.table_exists("integration", "multi_source_logs")
        
        # Verify log data
        log_data = mock_spark_session.storage.query_table("integration", "multi_source_logs")
