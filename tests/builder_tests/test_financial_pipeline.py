...
class TestFinancialPipeline:
    ...
    def test_complete_financial_transaction_pipeline_execution(self, mock_spark_session, data_generator, test_assertions):
        """Test complete financial pipeline: transactions → validation → fraud detection → compliance."""
        
        # Create realistic financial data
        transactions_df = data_generator.create_financial_transactions(mock_spark_session, num_transactions=100)
        ...
        def validate_transactions_transform(spark, df, silvers):
            ...
            return (df
                ...
                .withColumn("transaction_date", df.timestamp_parsed.cast("date"))
                ...
                .withColumn("is_weekend", df.day_of_week.isin([1, 7]))  # Sunday=1, Saturday=7
                ...
                .select("transaction_id", "account_id", "amount", "transaction_type",
                       "timestamp_parsed", "transaction_date", "hour_of_day", "day_of_week",
                       "is_weekend", "is_high_value", "is_off_hours", "merchant", "category", "fraud_score")
            )
        ...
        def enrich_accounts_transform(spark, df, silvers):
            ...
            return (df
                .withColumn("opening_date_parsed", df.opening_date.cast("date"))
                .withColumn("account_age_days", df.current_date().datediff(df.opening_date_parsed))
                ...
                .select("account_id", "account_type", "balance", "status", 
                       "opening_date_parsed", "account_age_days", "is_premium", "is_active")
            )
            ...
            if transactions is not None and accounts is not None:
                ...
                enriched_transactions = (transactions
                    ...
                    .withColumn("fraud_risk_score",
                              F.when(F.col("amount") > 0, 0.8).otherwise(0.9)
                              .when((df.is_high_value & df.is_off_hours), 0.7)
                              .when((df.is_high_value & df.is_weekend), 0.6)
                              .when(df.amount > df.balance * 0.5, 0.5)
                              .otherwise(df.fraud_score))
                ...
                return enriched_transactions.select(
                    ...
                    "transaction_date", "merchant", "category", "fraud_risk_score",
                    "fraud_risk_level", "requires_review", "account_type", "balance"
                )
            else:
                return spark.createDataFrame([], ["transaction_id", "account_id", "amount", "transaction_type", "transaction_date", "merchant", "category", "fraud_risk_score", "fraud_risk_level", "requires_review", "account_type", "balance"])
            ...
            if fraud_data is not None:
                ...
                return (fraud_data
                    ...
                    .agg(
                        ...
                        df.amount.sum().alias("total_amount"),
                        F.count(df.when(df.fraud_risk_level == "high", 1)).alias("high_risk_count"),
                        F.count(df.when(df.requires_review, 1)).alias("review_required_count"),
                        df.fraud_risk_score.avg().alias("avg_fraud_score"),
                        df.amount.max().alias("max_transaction_amount")
                    )
                    .withColumn("high_risk_percentage", 
                              df.high_risk_count / df.total_transactions * 100)
                ...
                return spark.createDataFrame([], ["transaction_date", "account_type", "total_transactions", "total_amount", "high_risk_count", "review_required_count", "avg_fraud_score", "max_transaction_amount", "high_risk_percentage", "compliance_status"])
        ...
        def account_balance_analytics_transform(spark, silvers):
            ...
            if transactions is not None and accounts is not None:
                ...
                account_analytics = (transactions
                    .groupBy("account_id")
                    .agg(
                        F.count("*").alias("transaction_count"),
                        df.amount.sum().alias("total_transacted"),
                        df.amount.avg().alias("avg_transaction_amount"),
                        df.amount.max().alias("max_transaction"),
                        df.amount.min().alias("min_transaction"),
                        df.merchant.nunique().alias("unique_merchants"),
                        df.category.nunique().alias("transaction_categories")
                    )
                )
                
                return (accounts
                    .join(account_analytics, "account_id", "left")
                    .withColumn("transaction_velocity", 
                              df.total_transacted / F.greatest(df.account_age_days, 1))
                    .withColumn("account_health_score",
                              F.when(F.col("is_active") & (F.col("balance") > 0), 100)
                              .when(df.is_active & (df.balance == 0), 50)
                              .otherwise(0))
                    .select("account_id", "account_type", "balance", "is_active", "is_premium",
                           "transaction_count", "total_transacted", "avg_transaction_amount",
                           "transaction_velocity", "account_health_score", "unique_merchants")
                )
            else:
                return spark.createDataFrame([], ["account_id", "account_type", "balance", "is_active", "is_premium", "transaction_count", "total_transacted", "avg_transaction_amount", "transaction_velocity", "account_health_score", "unique_merchants"])
    ...
    def test_fraud_detection_scenarios(self, mock_spark_session, data_generator, test_assertions):
        ...
        suspicious_transactions = mock_spark_session.createDataFrame([
            ("TXN-001", "ACC-001", 5000.00, "debit", "2024-01-01T02:00:00", "Unknown Merchant", "cash_advance", 0.9),  # High fraud score
            ("TXN-002", "ACC-001", 2000.00, "debit", "2024-01-01T03:00:00", "ATM", "cash_advance", 0.8),  # Multiple high-value transactions
            ("TXN-003", "ACC-002", 50.00, "debit", "2024-01-01T10:00:00", "Grocery Store", "groceries", 0.1),  # Normal transaction
            ("TXN-004", "ACC-003", 10000.00, "debit", "2024-01-01T23:30:00", "Online Casino", "entertainment", 0.95),  # Very suspicious
        ], ["transaction_id", "account_id", "amount", "transaction_type", "timestamp", "merchant", "category", "fraud_score"])
        ...
        def fraud_analysis_transform(spark, silvers):
            ...
            if transactions is not None and accounts is not None:
                ...
                return spark.createDataFrame([], ["transaction_id", "account_id", "amount", "is_suspicious", "risk_category", "requires_immediate_action"])
    ...
    def test_compliance_monitoring(self, mock_spark_session, data_generator, test_assertions):
        ...
        compliance_transactions = mock_spark_session.createDataFrame([
            ("TXN-001", "ACC-001", 10000.00, "debit", "2024-01-01T10:00:00", "Bank Transfer", "transfer", 0.1),  # Large transaction
            ("TXN-002", "ACC-001", 5000.00, "debit", "2024-01-01T11:00:00", "Cash Withdrawal", "cash_advance", 0.2),
            ("TXN-003", "ACC-002", 15000.00, "debit", "2024-01-01T12:00:00", "Wire Transfer", "transfer", 0.05),  # Very large transaction
        ], ["transaction_id", "account_id", "amount", "transaction_type", "timestamp", "merchant", "category", "fraud_score"])
        ...
        def compliance_monitoring_transform(spark, silvers):
            ...
            if transactions is not None:
                return (transactions
                    .withColumn("is_large_transaction", F.col("amount") >= 10000)
                    .withColumn("is_suspicious_pattern", 
                              (F.col("amount") >= 10000) & (F.col("category") == "transfer"))
                    .withColumn("compliance_flag",
                              F.when(F.col("amount") >= 15000, "SAR_REQUIRED")  # Suspicious Activity Report
                              .when(F.col("amount") >= 10000, "MONITOR")
                              .otherwise("NORMAL"))
                    .withColumn("reporting_required", df.compliance_flag.isin(["SAR_REQUIRED", "MONITOR"]))
                )
            else:
                return spark.createDataFrame([], ["transaction_id", "account_id", "amount", "is_large_transaction", "is_suspicious_pattern", "compliance_flag", "reporting_required"])
    ...
    def test_financial_audit_logging(self, mock_spark_session, data_generator, log_writer_config, test_assertions):
        """Test comprehensive audit logging for financial pipeline."""
        
        # Create test data
        transactions_df = data_generator.create_financial_transactions(mock_spark_session, num_transactions=50)
        
        # Setup schemas
        mock_spark_session.storage.create_schema("bronze")
        mock_spark_session.storage.create_schema("silver")
        mock_spark_session.storage.create_schema("gold")
        mock_spark_session.storage.create_schema("audit")
        ...
        builder.add_gold_transform(
            ...
            transform=lambda spark, silvers: silvers["validated_transactions"].agg(F.count("*").alias("total_transactions")),
        ...
        # Execute pipeline
        result = pipeline.run_initial_load(bronze_sources={"transactions": transactions_df})
        ...
        assert mock_spark_session.storage.table_exists("audit", "financial_audit_logs")
        
        # Verify audit log data
        audit_data = mock_spark_session.storage.query_table("audit", "financial_audit_logs")
