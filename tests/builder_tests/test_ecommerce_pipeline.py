...
class TestEcommercePipeline:
    ...
    def test_complete_ecommerce_pipeline_execution(self, mock_spark_session, data_generator, test_assertions):
        """Test complete e-commerce pipeline: orders → customer profiles → sales analytics."""
        
        # Create realistic test data
        orders_df = data_generator.create_ecommerce_orders(mock_spark_session, num_orders=50)
        customers_df = data_generator.create_customer_data(mock_spark_session, num_customers=25)
    ...
    def test_incremental_order_processing(self, mock_spark_session, data_generator, test_assertions):
        """Test incremental processing of new orders."""
        
        # Create initial data
        initial_orders = data_generator.create_ecommerce_orders(mock_spark_session, num_orders=20)
        ...
        builder.add_gold_transform(
            name="order_summary",
            transform=lambda spark, silvers: silvers["clean_orders"].agg(F.count("*").alias("total_orders")),
        ...
        new_orders = data_generator.create_ecommerce_orders(mock_spark_session, num_orders=10)
    ...
    def test_logging_and_monitoring(self, mock_spark_session, data_generator, log_writer_config, test_assertions):
        """Test comprehensive logging and monitoring with LogWriter."""
        
        # Create test data
        orders_df = data_generator.create_ecommerce_orders(mock_spark_session, num_orders=30)
