...
class TestIotPipeline:
    ...
    def test_complete_iot_sensor_pipeline_execution(self, mock_spark_session, data_generator, test_assertions):
        """Test complete IoT pipeline: sensor readings → anomaly detection → device health analytics."""
        
        # Create realistic sensor data
        sensor_data = data_generator.create_iot_sensor_data(mock_spark_session, num_readings=100)
        ...
        def clean_sensor_data_transform(spark, df, silvers):
            ...
            return (df
                ...
                .select("sensor_id", "timestamp_parsed", "hour", "date", 
        ...
        def hourly_aggregates_transform(spark, df, silvers):
            ...
            return (df_with_time
                .groupBy("sensor_id", "date", "hour", "location")
                .agg(
        ...
        def device_health_scores_transform(spark, silvers):
            ...
            if hourly_data is not None:
                ...
                window_spec = Window.partitionBy("sensor_id").orderBy("date", "hour")
                
                return (hourly_data
                    .withColumn("temp_stability", F.lit(100) - F.col("temp_stddev"))  # Higher stability = better health
                    .withColumn("data_completeness", F.col("reading_count") / F.lit(60) * F.lit(100))  # Expected 60 readings per hour
                    .withColumn("health_score", 
                               (F.col("temp_stability") * F.lit(0.6) + F.col("data_completeness") * F.lit(0.4)))
                    .withColumn("health_status", 
                               ...
                               .otherwise("poor"))
                    .select("sensor_id", "date", "hour", "location", "health_score", 
                           "health_status", "temp_stability", "data_completeness")
                )
            else:
                return spark.createDataFrame([], ["sensor_id", "date", "hour", "location", "health_score", "health_status", "temp_stability", "data_completeness"])
        ...
        def location_analytics_transform(spark, silvers):
            ...
            if hourly_data is not None:
                return (hourly_data
                    ...
                    .withColumn("sensor_density", F.col("total_readings") / F.col("active_sensors"))
                    .orderBy("location", "date")
                )
            else:
                return spark.createDataFrame([], ["location", "date", "active_sensors", "location_avg_temp", "location_avg_humidity", "total_readings", "sensor_density"])
    ...
    def test_incremental_sensor_processing(self, mock_spark_session, data_generator, test_assertions):
        """Test incremental processing of new sensor readings."""
        
        # Create initial sensor data
        initial_data = data_generator.create_iot_sensor_data(mock_spark_session, num_readings=50)
        ...
        builder.add_silver_transform(
            ...
            transform=lambda spark, df, silvers: df.filter(F.col("device_status") == "active"),
        ...
        builder.add_gold_transform(
            name="sensor_summary",
            transform=lambda spark, silvers: silvers["clean_readings"].agg(F.count("*").alias("total_readings")),
        ...
        new_data = data_generator.create_iot_sensor_data(mock_spark_session, num_readings=25)
    ...
    def test_anomaly_detection_pipeline(self, mock_spark_session, data_generator, test_assertions):
        """Test anomaly detection in sensor data."""
        
        # Create sensor data with some anomalies
        normal_data = data_generator.create_iot_sensor_data(mock_spark_session, num_readings=80)
        
        # Add some anomalous readings
        anomaly_data = mock_spark_session.createDataFrame([
            ("SENSOR-99", "2024-01-01T12:00:00", 150.0, 50.0, 1013.0, "Building-5", "active"),  # Extreme temperature
            ("SENSOR-99", "2024-01-01T12:01:00", -100.0, 50.0, 1013.0, "Building-5", "active"),  # Extreme temperature
            ("SENSOR-99", "2024-01-01T12:02:00", 25.0, 150.0, 1013.0, "Building-5", "active"),  # Invalid humidity
        ], ["sensor_id", "timestamp", "temperature", "humidity", "pressure", "location", "device_status"])
        ...
        def anomaly_detection_transform(spark, df, silvers):
            ...
            return (df
                .withColumn("is_temperature_anomaly", 
        ...
        def anomaly_summary_transform(spark, silvers):
            ...
            if anomaly_data is not None:
                return (anomaly_data
                    .groupBy("sensor_id", "anomaly_type")
                    .agg(F.count("*").alias("anomaly_count"))
                    .filter(F.col("anomaly_type") != "normal")
                )
            else:
                return spark.createDataFrame([], ["sensor_id", "anomaly_type", "anomaly_count"])
        ...
        # Execute pipeline
        result = pipeline.run_initial_load(
            bronze_sources={"sensor_readings": all_data}
        )
    ...
    def test_performance_monitoring(self, mock_spark_session, data_generator, log_writer_config, test_assertions):
        """Test performance monitoring and logging for IoT pipeline."""
        
        # Create large sensor dataset
        sensor_data = data_generator.create_iot_sensor_data(mock_spark_session, num_readings=200)
        ...
        builder.add_gold_transform(
            name="device_metrics",
            transform=lambda spark, silvers: silvers["processed_readings"].agg(F.count("*").alias("total_readings")),
            rules={"total_readings": ["not_null"]},
        ...
        # Execute pipeline
        result = pipeline.run_initial_load(bronze_sources={"sensor_readings": sensor_data})
        ...
        log_data = mock_spark_session.storage.query_table("analytics", "iot_pipeline_logs")
