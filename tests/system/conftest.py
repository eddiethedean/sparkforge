...
def _ensure_java_home():
    """Ensure JAVA_HOME is set for Spark to work."""
    if not os.environ.get("JAVA_HOME"):
        # Try to find Java 17 via Homebrew
        try:
            ...
            java_prefix = result.stdout.strip()
            java_home = f"{java_prefix}/libexec/openjdk.jdk/Contents/Home"
            if os.path.exists(java_home):
                os.environ["JAVA_HOME"] = java_home
                os.environ["PATH"] = f"{java_home}/bin:{os.environ.get('PATH', '')}"
        except (subprocess.CalledProcessError, FileNotFoundError):
            pass  # Fall back to system Java if available
