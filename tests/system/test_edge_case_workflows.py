...
def create_test_data(
    ...
    for i in range(num_records):
        ...
        action = (
            ["click", "view", "purchase", "add_to_cart"][i % 4]
            if not (include_nulls and i % 7 == 0)
            else None
        )
...
class TestEdgeCaseWorkflows:
    ...
    def test_empty_dataframe_handling(self, mock_spark_session):
        ...
        gold_table = f"gold_user_metrics_{identifier}"
    ...
    def test_schema_mismatch_handling(self, mock_spark_session):
        ...
        result1 = pipeline.run_initial_load(bronze_sources={"raw_events": source_df})
    ...
    def test_writer_failure_recovery(self, mock_spark_session):
        ...
        try:
            ...
            write_result = log_writer.write_log_rows(
                log_rows, run_id=f"test_run_{uuid4().hex[:8]}"
            )
    ...
    def test_concurrent_initial_runs(self, mock_spark_session):
        ...
        # Store the table name to use it later
        silver_table_name = f"silver_processed_events_{uuid4().hex[:8]}"
        builder.add_silver_transform(
            name="processed_events",
            source_bronze="raw_events",
            transform=silver_transform,
