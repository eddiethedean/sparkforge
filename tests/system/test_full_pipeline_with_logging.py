...
def assert_step_result(step_result, expected_rows: Optional[int] = None, 
    ...
    assert 0.0 <= step_result.validation_rate <= 100.0, "Validation rate should be 0-100"
...
def assert_log_entry(log_row: Dict, step_name: str, run_id: str, run_mode: str, 
...
class TestFullPipelineWithLogging:
    ...
    def test_full_pipeline_with_logging(
        self, mock_spark_session, log_writer
    ):
        """Test complete pipeline with 2 bronze, 2 silver, 1 gold steps and logging."""
        schema = "test_schema"
        
        # Create initial test data
        initial_data_source1 = create_test_data_initial(mock_spark_session, num_records=100)
        initial_data_source2 = create_test_data_initial(mock_spark_session, num_records=100)
        
        initial_df1 = mock_spark_session.createDataFrame(
            initial_data_source1,
            ["user_id", "action", "timestamp", "value"]
        )
        initial_df2 = mock_spark_session.createDataFrame(
            initial_data_source2,
            ["user_id", "action", "timestamp", "value"]
        )
        ...
        assert bronze_step1["rows_processed"] == 100, "Bronze step 1 should process 100 rows"
        assert bronze_step2["status"] == "completed", "Bronze step 2 should complete"
        assert bronze_step2["rows_processed"] == 100, "Bronze step 2 should process 100 rows"
        ...
        assert silver_step1["rows_written"] == 100, "Silver step 1 should write 100 rows"
        assert silver_step2["status"] == "completed", "Silver step 2 should complete"
        assert silver_step2["rows_written"] == 100, "Silver step 2 should write 100 rows"
        ...
        assert silver_count == 200, f"Expected 200 rows in silver table, got {silver_count}"
        ...
        assert initial_log_count == 6, \
            f"Expected 6 log rows, got {initial_log_count}"
        
        # Verify log entries
        initial_logs = get_log_entries_by_run(log_df, initial_run_id)
        # create_log_rows_from_pipeline_report creates 1 main row + 5 step rows = 6 total
        # All rows have step_name, so we just check we have 6 total
        assert len(initial_logs) == 6, f"Should have 6 log entries for initial run (1 main + 5 steps), got {len(initial_logs)}"
        
        # Filter to step rows only (exclude main pipeline row which might have different step_name)
        step_logs = [log for log in initial_logs if log.get("step_name") and log["step_name"] not in ["", None, "pipeline"]]
        # We expect 5 step rows (2 bronze + 2 silver + 1 gold)
        assert len(step_logs) >= 5, f"Should have at least 5 step log entries for initial run, got {len(step_logs)}"
        ...
        assert "processed_events_1" in step_names, "Should have log for processed_events_1"
        assert "processed_events_2" in step_names, "Should have log for processed_events_2"
        ...
        assert_log_entry(bronze1_log, "events_source1", initial_run_id, "initial", expected_rows=None)
        # Check rows_processed from the actual step result data
        assert bronze1_log.get("rows_processed", 0) >= 0, "Bronze step 1 should have rows_processed"
        bronze2_log = next(l for l in step_logs if l["step_name"] == "events_source2")
        assert_log_entry(bronze2_log, "events_source2", initial_run_id, "initial", expected_rows=None)
        assert bronze2_log.get("rows_processed", 0) >= 0, "Bronze step 2 should have rows_processed"
        
        # Verify silver step logs
        silver1_log = next(l for l in step_logs if l["step_name"] == "processed_events_1")
        assert_log_entry(silver1_log, "processed_events_1", initial_run_id, "initial", expected_rows=None)
        assert silver1_log.get("rows_written", 0) >= 0, "Silver step 1 should have rows_written"
        silver2_log = next(l for l in step_logs if l["step_name"] == "processed_events_2")
        assert_log_entry(silver2_log, "processed_events_2", initial_run_id, "initial", expected_rows=None)
        assert silver2_log.get("rows_written", 0) >= 0, "Silver step 2 should have rows_written"
        ...
        incremental_data_source1 = create_test_data_incremental(mock_spark_session, num_records=50)
        incremental_data_source2 = create_test_data_incremental(mock_spark_session, num_records=50)
        
        incremental_df1 = mock_spark_session.createDataFrame(
            incremental_data_source1,
            ["user_id", "action", "timestamp", "value"]
        )
        incremental_df2 = mock_spark_session.createDataFrame(
            incremental_data_source2,
            ["user_id", "action", "timestamp", "value"]
        )
        ...
        assert inc_bronze_step1["status"] == "completed", "Incremental bronze step 1 should complete"
        assert inc_bronze_step1["rows_processed"] == 50, "Incremental bronze step 1 should process 50 rows"
        assert inc_bronze_step2["status"] == "completed", "Incremental bronze step 2 should complete"
        assert inc_bronze_step2["rows_processed"] == 50, "Incremental bronze step 2 should process 50 rows"
        ...
        silver_table_after = mock_spark_session.table(f"{schema}.silver_processed_events")
        ...
        log_result_inc = log_writer.write_log_rows(log_rows_inc, run_id=incremental_run_id)
        ...
        assert len(incremental_logs) == 6, f"Should have 6 log entries for incremental run (1 main + 5 steps), got {len(incremental_logs)}"
        
        # Filter to step rows only
        step_logs_inc = [log for log in incremental_logs if log.get("step_name") and log["step_name"] not in ["", None, "pipeline"]]
        assert len(step_logs_inc) >= 5, f"Should have at least 5 step log entries for incremental run, got {len(step_logs_inc)}"
        ...
        assert len(all_initial_logs) == 6, "Initial run logs should still exist (6 rows)"
        
        # Verify incremental step logs (use step_logs_inc which excludes main pipeline row)
        inc_bronze1_log = next(l for l in step_logs_inc if l["step_name"] == "events_source1")
        assert_log_entry(inc_bronze1_log, "events_source1", incremental_run_id, "incremental", expected_rows=None)
        assert inc_bronze1_log.get("rows_processed", 0) >= 0, "Incremental bronze step 1 should have rows_processed"
        inc_bronze2_log = next(l for l in step_logs_inc if l["step_name"] == "events_source2")
        assert_log_entry(inc_bronze2_log, "events_source2", incremental_run_id, "incremental", expected_rows=None)
        assert inc_bronze2_log.get("rows_processed", 0) >= 0, "Incremental bronze step 2 should have rows_processed"
        
        # Verify timestamps are later for incremental run (if timestamps are strings, compare as strings)
        initial_timestamps = [log.get("timestamp") for log in all_initial_logs if log.get("timestamp")]
        incremental_timestamps = [log.get("timestamp") for log in incremental_logs if log.get("timestamp")]
        ...
        final_silver_table = mock_spark_session.table(f"{schema}.silver_processed_events")
        ...
        assert final_gold_count > 0, "Gold table should have aggregated rows after incremental run"
        
        # Verify log table structure and data integrity
        log_schema = log_df_after.schema
        required_columns = ["step_name", "run_id", "run_mode", "rows_processed", "rows_written", "phase"]
