...
from pipeline_builder_base.writer.models import LogRow, WriteMode, WriterConfig, WriterMetrics
...
class SqlLogWriter(BaseLogWriter):
    ...
    def __init__(
        ...
        if schema is None or table_name is None:
            if config is None:
                raise ValueError("Either (schema, table_name) or config must be provided")
    ...
    def _write_log_rows(self, log_rows: list[LogRow], mode: WriteMode) -> None:
        ...
        """
        try:
            from sqlalchemy import Table, Column, String, Integer, Float, Boolean, DateTime, MetaData, Text
            ...
            log_table = Table(
                ...
                schema=self.schema,
                autoload_with=self.session.bind if hasattr(self.session, "bind") else None,
            )
            ...
            self.logger.info(f"Successfully wrote {len(log_rows)} log rows to {self.table_fqn}")
    ...
    def _create_table(self, sample_rows: list[LogRow]) -> None:
        ...
        try:
            ...
            # Create table
            metadata.create_all(self.session.bind if hasattr(self.session, "bind") else None)
            self.session.commit()
