...
class SqlExecutionEngine:
    ...
    def __init__(
        ...
        self.is_async = is_async_engine(session.bind if hasattr(session, "bind") else None)
    ...
    def _execute_silver_step(
        ...
        if bronze_key not in context:
            raise ValueError(f"Bronze source '{step.source_bronze}' not found in context")
    ...
    def execute_pipeline(
        ...
        for group in execution_groups:
            if self.is_async and self.config.parallel.enabled:
                ...
                step_results.extend(
                    self._execute_group_async(group, bronze_steps, silver_steps, gold_steps, execution_context, mode)
                )
            else:
    ...
    def _execute_group_async(
        ...
        self.logger.warning("Async parallel execution not fully implemented, using sequential")
        results = []
        for step_name in group:
            step = (
                bronze_steps.get(step_name)
