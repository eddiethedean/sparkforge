...
def step_validate_bronze_data(context):
    ...
    try:
        context.validation_result = assess_data_quality(
            context.bronze_data, context.validation_rules
        )
...
def step_validate_data(context):
    ...
    try:
        context.validation_result = assess_data_quality(
            context.test_data, context.validation_rules
        )
        context.validation_success = True
    except Exception as e:
        context.validation_error = str(e)
...
def step_validate_incremental_data(context):
    ...
    try:
        context.validation_result = assess_data_quality(
            context.new_data, context.validation_rules
        )
        context.validation_success = True
    except Exception as e:
        context.validation_error = str(e)
...
def step_validation_should_pass_for_valid_records(context):
    ...
    assert context.validation_success, (
        f"Validation failed: {getattr(context, 'validation_error', 'Unknown error')}"
    )
...
def step_invalid_records_should_be_flagged(context):
    ...
    assert context.validation_result.get("invalid_rows", 0) > 0, (
        "No invalid records were flagged"
    )
...
def step_should_receive_validation_report(context):
    ...
    for field in expected_fields:
        assert field in context.validation_result, (
            f"Validation report missing field: {field}"
        )
...
def step_pipeline_should_continue_with_valid_records(context):
    ...
    assert context.validation_result.get("valid_rows", 0) > 0, (
...
def step_only_new_data_should_be_validated(context):
    ...
    validation_count = context.validation_result.get(
        "valid_rows", 0
    ) + context.validation_result.get("invalid_rows", 0)

    assert validation_count <= new_data_count, (
        "Validating more data than expected for incremental processing"
    )
...
def step_validation_should_be_consistent(context):
    ...
    assert "validation_rate" in context.validation_result, (
        "No validation rate in result"
    )
...
def step_should_get_alerts_for_significant_changes(context):
    ...
    if current_rate < alert_threshold:
        ...
        print(
            f"No alert needed: Quality above threshold ({current_rate}% >= {alert_threshold}%)"
        )
