...
def step_have_invalid_pipeline_configuration(context):
    ...
    context.invalid_config = {
        ...
        'silver_steps': [{'name': 'test', 'source_bronze': 'nonexistent'}],  # Invalid source
        'gold_steps': [],
...
def step_have_pipeline_requiring_more_resources(context):
    ...
    context.resource_error = "Insufficient resources: requires 8GB memory, 16 CPU cores, 100GB disk space"
...
def step_have_pipeline_requiring_network_access(context):
    ...
    context.connectivity_error = "Network connectivity issues: unable to reach external services"
...
def step_have_pipeline_with_schema_changes(context):
    ...
    context.schema_conflict = "Schema evolution conflict: new columns added, existing columns removed"
...
def step_execute_pipeline_with_resource_issues(context):
    ...
    try:
        ...
        if context.resource_requirements['memory'] > context.available_resources['memory']:
            raise ResourceError(
...
def step_execute_pipeline_with_schema_issues(context):
    ...
    try:
        ...
        if context.schema_changes['new_columns'] or context.schema_changes['removed_columns']:
...
def step_pipeline_should_not_crash(context):
    ...
    assert context.error_handled, "Pipeline crashed instead of handling error gracefully"
...
def step_should_receive_clear_error_message(context):
    ...
    assert hasattr(context, 'pipeline_error') or hasattr(context, 'config_error') or \
    ...
    error_obj = getattr(context, 'pipeline_error', None) or \
                ...
                getattr(context, 'resource_error', None) or \
...
def step_should_receive_configuration_error(context):
    ...
    assert isinstance(context.config_error, PipelineConfigurationError), "Wrong error type"
...
def step_error_should_be_caught_early(context):
    ...
    assert hasattr(context, 'config_validation_success'), "No configuration validation performed"
    assert not context.config_validation_success, "Configuration validation should have failed"
...
def step_errors_should_include_debugging_information(context):
    ...
    for error_obj in error_objects:
        if error_obj is not None:
            ...
            assert hasattr(error_obj, 'suggestions'), f"Error {type(error_obj).__name__} missing suggestions"
