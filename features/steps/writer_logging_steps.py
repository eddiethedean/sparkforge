...
def step_write_execution_result_to_logs(context):
    ...
    try:
        context.writer_result = context.writer.write_execution_result(context.execution_result)
...
def step_export_logs_for_analysis(context):
    ...
    try:
        ...
        context.export_result = {
            ...
            'fields_included': ['execution_time', 'success', 'validation_rate', 'error_message']
        }
...
def step_log_should_contain_execution_details(context):
    ...
    assert context.execution_result.total_rows_processed is not None, "No rows processed count captured"
...
def step_step_logs_should_contain_timing_information(context):
    ...
    for phase in ['bronze_results', 'silver_results', 'gold_results']:
        if hasattr(context.execution_result, phase):
            results = getattr(context.execution_result, phase)
            for step_name, step_result in results.items():
                assert step_result.execution_time is not None, f"No execution time for step {step_name}"
                assert step_result.execution_time > 0, f"Invalid execution time for step {step_name}"
...
def step_step_logs_should_contain_data_quality_metrics(context):
    ...
    for phase in ['bronze_results', 'silver_results', 'gold_results']:
        if hasattr(context.execution_result, phase):
            results = getattr(context.execution_result, phase)
            for step_name, step_result in results.items():
                assert step_result.validation_rate is not None, f"No validation rate for step {step_name}"
                assert 0 <= step_result.validation_rate <= 100, f"Invalid validation rate for step {step_name}"
...
def step_step_logs_should_contain_resource_usage(context):
    ...
    for phase in ['bronze_results', 'silver_results', 'gold_results']:
        if hasattr(context.execution_result, phase):
            results = getattr(context.execution_result, phase)
            for step_name, step_result in results.items():
                assert step_result.input_rows is not None, f"No input rows for step {step_name}"
                assert step_result.output_rows is not None, f"No output rows for step {step_name}"
...
def step_quality_alerts_should_be_generated(context):
    ...
    if validation_rate < 90.0:  # Threshold
        ...
        print(f"No quality alert needed: Validation rate {validation_rate}% above threshold 90%")
...
def step_should_receive_logs_in_structured_format(context):
    ...
    assert context.export_result['export_format'] == 'structured', "Export format is not structured"
...
def step_export_should_include_all_relevant_fields(context):
    ...
    for field in expected_fields:
        assert field in context.export_result['fields_included'], f"Field {field} not included in export"
