...
def store_failures(mode: str = "pyspark") -> List[str]:
    ...
    for line in output.splitlines():
        if "FAILED" in line or "ERROR" in line:
            ...
            if parts:
                test_path = parts[0]
                if test_path.startswith("tests/") and ("FAILED" in line or "ERROR" in line):
...
def rerun_failures(mode: str = "pyspark") -> bool:
    ...
    if result.returncode == 0:
        ...
        print("⚠️  Some tests still failing. Run 'store' again to update the failure list")
        return False
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python scripts/store_and_rerun_failed_tests.py {store|rerun|clear} [mode]")
        print("")
        print("Commands:")
        print("  store <mode>  - Run all tests and store failures (default: pyspark)")
        print("  rerun <mode>  - Rerun only previously stored failures (default: pyspark)")
