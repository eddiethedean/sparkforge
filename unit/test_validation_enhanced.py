...
class TestValidationWithMockFunctions:
    ...
    def setup_method(self):
        ...
        self.mock_df = self.mock_spark.createDataFrame(self.sample_data, self.sample_schema)
    ...
    def test_convert_rule_to_expression_with_mock_functions(self):
        ...
        # MockColumnOperation should have isNotNull method
        ...
        expr = _convert_rule_to_expression("non_negative", "salary", self.mock_functions)
        assert expr is not None
    ...
    def test_mock_functions_protocol_compliance(self):
        ...
        assert callable(self.mock_functions.col)
        assert callable(self.mock_functions.lit)
        assert callable(self.mock_functions.current_timestamp)
...
class TestPipelineBuilderWithMockFunctions:
    ...
    def test_pipeline_builder_static_methods_with_mock_functions(self):
        ...
        rules = PipelineBuilder.positive_number_rules(["age", "salary"], self.mock_functions)
        assert isinstance(rules, dict)
        assert "age" in rules
...
class TestMockFunctionsIntegration:
    ...
    def test_functions_protocol_type_checking(self):
        ...
        protocol_methods = [
            'col', 'lit', 'current_timestamp', 'count', 'countDistinct', 
            'max', 'sum', 'when', 'length'
        ]
        ...
        # (We can't use isinstance with Protocol in Python 3.8, but we can test compatibility)
        try:
