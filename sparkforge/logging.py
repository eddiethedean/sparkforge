...
class PipelineLogger:
    ...
    def pipeline_end(self, pipeline_name: str, duration: float, success: bool = True) -> None:
        """Log pipeline end."""
    ...
    def step_complete(self, stage: str, step: str, duration: float, rows: int = 0) -> None:
        """Log step completion."""
        self.info(f"✅ Completed {stage.upper()} step: {step} ({duration:.2f}s, {rows:,} rows)")
    
    def step_failed(self, stage: str, step: str, error: str, duration: float = 0) -> None:
        """Log step failure."""
        self.error(f"❌ Failed {stage.upper()} step: {step} ({duration:.2f}s) - {error}")
    
    def validation_passed(self, stage: str, step: str, rate: float, threshold: float) -> None:
        """Log validation success."""
        self.info(f"✅ Validation passed for {stage}:{step} - {rate:.2f}% >= {threshold:.2f}%")
    
    def validation_failed(self, stage: str, step: str, rate: float, threshold: float) -> None:
        """Log validation failure."""
        self.warning(f"❌ Validation failed for {stage}:{step} - {rate:.2f}% < {threshold:.2f}%")
    
    def performance_metric(self, metric_name: str, value: float, unit: str = "s") -> None:
        """Log performance metric."""
        self.info(f"📊 {metric_name}: {value:.2f}{unit}")
    
    # Context management
    @contextmanager
    def context(self, **context_data: Union[str, int, float, bool, List[str], Dict[str, str], None]) -> Generator[None, None, None]:
