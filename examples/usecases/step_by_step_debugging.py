...
def main():
    ...
    spark = SparkSession.builder \
        .appName("Step-by-Step Debugging") \
        .master("local[*]") \
    ...
    try:
        ...
        print("\n📋 Data Quality Issues:")
        print(f"Records with null user_id: {sample_df.filter(F.col('user_id').isNull()).count()}")
        print(f"Records with negative amount: {sample_df.filter(F.col('amount') < 0).count()}")
        ...
        def daily_summary(spark, silvers):
            ...
            return (transactions_df
                .groupBy("category", F.date_trunc("day", "timestamp").alias("date"))
                .agg(
        ...
        bronze_result = pipeline.execute_bronze_step("transactions", input_data=sample_df)
        ...
        print(f"Silver validation passed: {silver_result.validation_result.validation_passed}")
        print(f"Silver validation rate: {silver_result.validation_result.validation_rate:.2f}%")
        ...
        if silver_result.status.value == "completed":
            ...
            # Check data quality
            print("\n📈 Silver data quality:")
            print(f"High value transactions: {silver_output.filter(F.col('amount_category') == 'high_value').count()}")
            print(f"Weekend transactions: {silver_output.filter(F.col('is_weekend') == True).count()}")
        ...
        print(f"Gold validation passed: {gold_result.validation_result.validation_passed}")
        print(f"Gold validation rate: {gold_result.validation_result.validation_rate:.2f}%")
