...
def main():
    ...
    spark = SparkSession.builder \
        .appName("StepByStepExecutionExample") \
        .master("local[*]") \
    ...
    try:
        ...
        if silver_result.validation_result:
            print(f"   Validation passed: {silver_result.validation_result.validation_passed}")
        ...
        if gold_result.validation_result:
            print(f"   Validation passed: {gold_result.validation_result.validation_passed}")
        ...
        print("   Let's say we want to modify the Silver transform and re-run just that step...")
        ...
        def modified_silver_transform(spark, bronze_df):
            return (bronze_df
                    ...
                    .select("user_id", "action", "value", "event_date", "processed_at", "is_high_value")
                   )
        ...
        modified_silver_output = pipeline.create_step_executor().get_step_output("silver_events")
        ...
        for step_name, result in execution_state.items():
            print(f"   {step_name}: {result.status.value} ({result.duration_seconds:.2f}s)")
