...
def main():
    ...
    # Initialize Spark
    spark = (
        SparkSession.builder.appName("ImprovedUserExperienceExample")
        .master("local[*]")
    ...
    builder = PipelineBuilder.for_development(spark=spark, schema="improved_ux_schema")
    logger.info(
        f"✅ Development preset: Bronze={builder.config.thresholds.bronze}%, Silver={builder.config.thresholds.silver}%, Gold={builder.config.thresholds.gold}%"
    )
    ...
    builder.with_bronze_rules(
        name="events", rules=bronze_rules, incremental_col="timestamp"
    )
