...
def create_test_data(spark: SparkSession):
    ...
    df.write.mode("overwrite").option("path", "/tmp/sparkforge_test/raw_data/user_events").saveAsTable("raw_data.user_events")
...
def daily_metrics_transform(spark, silvers):
    ...
    return (daily_counts
        .join(user_segments, "user_id", "left")
    ...
    try:
        ...
        builder.add_silver_transform(
            ...
            rules={
                ...
                "value_category": [F.col("value_category").isin(["low", "medium", "high"])]
            },
        ...
        builder.add_silver_transform(
            ...
            rules={
                "user_id": [F.col("user_id").isNotNull()],
                "user_segment": [F.col("user_segment").isin(["basic", "standard", "premium"])],
        ...
        # Check raw data
        raw_count = spark.sql("SELECT COUNT(*) as count FROM raw_data.user_events").collect()[0]["count"]
        logger.info(f"  Raw Data (raw_data.user_events): {raw_count} records")
        
        # Check processed data
        try:
            processing_count = spark.sql("SELECT COUNT(*) as count FROM processing.clean_user_events").collect()[0]["count"]
            logger.info(f"  Clean Data (processing.clean_user_events): {processing_count} records")
        except Exception as e:
            logger.warning(f"  Could not read processing.clean_user_events: {e}")
        
        try:
            staging_count = spark.sql("SELECT COUNT(*) as count FROM staging.enriched_user_events").collect()[0]["count"]
            logger.info(f"  Enriched Data (staging.enriched_user_events): {staging_count} records")
        except Exception as e:
            logger.warning(f"  Could not read staging.enriched_user_events: {e}")
        
        try:
            analytics_count = spark.sql("SELECT COUNT(*) as count FROM analytics.user_daily_metrics").collect()[0]["count"]
            logger.info(f"  Analytics Data (analytics.user_daily_metrics): {analytics_count} records")
        except Exception as e:
        ...
        try:
            ...
            for row in sample_data:
                logger.info(f"  User {row['user_id']}: {row['total_events']} events on {row['event_date']} "
