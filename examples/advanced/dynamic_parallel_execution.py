...
from sparkforge import (
    PipelineBuilder, PipelineRunner, DynamicExecutionStrategy,
    DynamicParallelExecutor, ExecutionTask, TaskPriority,
    WorkerAllocationStrategy, create_execution_task, get_dynamic_executor
)
...
def heavy_aggregation(df: DataFrame) -> DataFrame:
    ...
    return (df
            .groupBy("department", "location", "salary_category")
            .agg(
...
def demonstrate_dynamic_parallel_execution():
    ...
    spark = SparkSession.builder \
        .appName("SparkForge Dynamic Parallel Execution") \
    ...
    try:
        ...
        builder.add_gold_transform(
            ...
            transform=lambda spark, silvers: heavy_aggregation(silvers["processed_employees"]),
            rules={
        ...
        for strategy in strategies:
            ...
            start_time = time.time()
            result = strategy_executor.execute_parallel(test_tasks, wait_for_completion=True)
            execution_time = time.time() - start_time
...
def demonstrate_advanced_features():
    ...
    # Initialize Spark
    spark = SparkSession.builder \
        .appName("SparkForge Advanced Features") \
        .getOrCreate()
    ...
    try:
        ...
        steps = {
            ...
            "complex_silver_step": {
                "step_type": "silver",
                "rules": {"id": ["not_null"], "name": ["not_null"], "value": ["not_null"]},
                "transform": complex_data_processing
            },
            "simple_silver_step": {
                "step_type": "silver",
        ...
        for step_name, step_analysis in analysis.items():
            ...
            print(f"    - Memory Requirement: {step_analysis.memory_requirement_mb:.0f}MB")
            print(f"    - CPU Requirement: {step_analysis.cpu_requirement_percent:.0f}%")
