...
def demonstrate_column_filtering():
    ...
    spark = SparkSession.builder \
        .appName("ColumnFilteringBehavior") \
        .master("local[*]") \
        .getOrCreate()
    ...
    try:
        ...
        schema = StructType([
            ...
            StructField("device_type", StringType(), True),  # This column won't have rules
        ])
...
def demonstrate_pipeline_impact():
    ...
    # Initialize Spark
    spark = SparkSession.builder \
        .appName("PipelineColumnFiltering") \
        .master("local[*]") \
        .getOrCreate()
    ...
    try:
        ...
        def silver_transform(spark, bronze_df):
            ...
            try:
                result = (bronze_df
                    ...
                    .select("user_id", "event_type", "value", "event_date", 
                           "is_mobile", "is_premium")
                )
                logger.info(f"Silver transform output columns: {result.columns}")
                return result
            except Exception as e:
                logger.error(f"Silver transform failed: {e}")
                logger.error("This happens when columns are filtered out by validation rules")
                raise
        ...
        logger.info("   Note: This will show how column filtering affects downstream steps")
        
        # Run pipeline
        result = pipeline.initial_load(bronze_sources={"user_events": df})
